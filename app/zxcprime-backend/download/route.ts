...
export interface VideoLink {
  ...
  url: string;
  resolution: number;
  size: string; // kept as string since API returns it that way
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const d of downloads) {
      workingProxy = await getWorkingProxy(String(d.url), proxies);
      if (workingProxy) break;
    }
    ...
    const proxiedLinks: VideoLink[] = downloads.map((d: any) => ({
      id: String(d.id),
      resolution: Number(d.resolution),
      size: String(d.size),
      url: `${workingProxy}?url=${encodeURIComponent(String(d.url))}`,
    }));
